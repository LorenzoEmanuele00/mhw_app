...
def main():
    ...
    for skill in skills_json:
        ...
        raw_desc = skill.get('descriptions', {}).get('en', None)
        skill_desc = raw_desc.replace('\r\n', ' ').replace('\r', ' ').replace('\n', ' ').strip() if raw_desc else None
        ...
        for rank in ranks:
            ...
            level_desc = raw_level_desc.replace('\r\n', ' ').replace('\r', ' ').replace('\n', ' ').strip() if raw_level_desc else None
    ...
    with open(skills_sql_path, 'w', encoding='utf-8') as f:
        ...
        f.write('INSERT INTO skills (id, slug, name, description, max_level, type1, type2) VALUES\n')
        rows = []
    ...
    with open(levels_sql_path, 'w', encoding='utf-8') as f:
        ...
        f.write('  (skill_id, level, description, pieces_required,\n')
